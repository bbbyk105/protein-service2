...
class AnalyzeReq(BaseModel):
    ...
    method: str = Field(
        "X-ray",
        description="Experimental method: X-ray | EM | NMR",
        examples=["X-ray", "EM", "NMR"]
    )
    seq_ratio: int = Field(
        20,
        ge=1,
        le=100,
        description="Sequence ratio parameter (研究室パラメータ)"
    )
    negative_pdbids: Optional[Union[List[str], str]] = Field(
        None,
        description="PDB IDs to exclude from analysis"
    )
...
def health():
    ...
    return {
        "status": "ok",
        "service": "Protein Structure Analysis",
        "version": "1.0.0"
    }
...
def analyze(req: AnalyzeReq):
    ...
    for uid in ids:
        ...
        print(f"✓ PDB IDs: {pdb_ids} ({len(pdb_ids)} entries)")
